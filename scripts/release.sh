#!/usr/bin/env bash
# Build a distributable Ugla.app + DMG locally: arm64, bundled signed
...
swift build --package-path "$APPDIR" -c release --product Ugla
BIN="$(swift build --package-path "$APPDIR" -c release --show-bin-path)/Ugla"
cp "$BIN" "$APP/Contents/MacOS/Ugla"
...
hdiutil create -volname "Ugla" -srcfolder "$STAGE" -ov -format UDZO "$DMG"
rm -rf "$STAGE"
